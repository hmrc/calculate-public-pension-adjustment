...
class CppaBaseController(cc: ControllerComponents) extends BackendController(cc) with Logging {
  def withValidJson[T](
    errMessage: String
    ...
    request.body.validate[T] match {
      ...
      case JsError(errors)     =>
        logger.error(s"Json parsing errors : $errors")
        Future.successful(BadRequest(s"Invalid $errMessage"))
    }
}
